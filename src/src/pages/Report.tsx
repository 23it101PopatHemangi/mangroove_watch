...
import { Camera, MapPin, Upload, CheckCircle, AlertTriangle, MessageSquare, Phone } from "lucide-react";
import { incidents, mangroveLocations } from "@/data/mangroveData";
import { useToast } from "@/hooks/use-toast";
import mangroveHero from "@/assets/mangrove-hero.jpg";
import { reportsAPI, alertsAPI, smsAPI } from "@/services/api";
import { useAuth } from "@/contexts/AuthContext";
import { useNavigate, Link } from "react-router-dom";

const Report = () => {
  const [reportType, setReportType] = useState<'photo' | 'sms'>('photo');
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (reportType === 'photo') {
      ...
      if (!phoneNumber || !description || !location.lat || !location.lng) {
        toast({
          title: "Missing information",
          description: "Please fill phone number, message, and location.",
          variant: "destructive",
        });
        return;
      }
    ...
    try {
      ...
      if (reportType === 'photo') {
        ...
        // Submit photo report to backend
        response = await reportsAPI.create(reportData);
      } else if (reportType === 'sms') {
        // Submit SMS report
        response = await smsAPI.sendReport(phoneNumber, description, `${lat},${lng}`);
        
        // Also save to regular reports database
        const reportData = {
      ...
      if (reportType === 'photo') {
        toast({
          title: "Photo Report submitted! 🎉",
          description: response.message || "You earned +15 points! AI validation in progress. Thank you for protecting our mangroves.",
        });
      } else {
        toast({
          title: "SMS Report stored! 📱",
          description: response.message || "SMS report saved to database successfully. Thank you for protecting our mangroves.",
        });
      }
          ...
          <CardContent>
            <form onSubmit={handleSubmit} className="space-y-6">
              ...
              <div className="space-y-2">
                <Label className="text-eco-forest font-medium">Report Type *</Label>
                <div className="flex space-x-4">
                  <Button
                    type="button"
                    variant={reportType === 'photo' ? 'default' : 'outline'}
                    onClick={() => setReportType('photo')}
                    className={`flex-1 ${reportType === 'photo' ? 'bg-eco-green text-white' : 'border-eco-sage text-eco-forest'}`}
                  >
                  ...
                  <Button
                    type="button"
                    variant={reportType === 'sms' ? 'default' : 'outline'}
                    onClick={() => setReportType('sms')}
                    className={`flex-1 ${reportType === 'sms' ? 'bg-blue-500 text-white' : 'border-eco-sage text-eco-forest'}`}
                  >
              ...
              {/* Photo Upload - Only show for photo reports */}
              {reportType === 'photo' && (
                <div className="space-y-2">
              ...
              {/* SMS Phone Number - Only show for SMS reports */}
              {reportType === 'sms' && (
                <div className="space-y-2">
                  <Label htmlFor="phoneNumber" className="text-eco-forest font-medium">
                    Phone Number *
                  </Label>
                  <Input
                    id="phoneNumber"
              ...
              >
                {isSubmitting ? (
                  <>
                    ...
                    {reportType === 'photo' ? 'Submitting Report...' : 'Sending SMS...'}
                  </>
                ) : (
                  <>
                    {reportType === 'photo' ? (
