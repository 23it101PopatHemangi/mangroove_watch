      ...
      try {
        ...
        if (response && response.data && Array.isArray(response.data)) {
          const backendReports = response.data.map((report: any) => ({
            ...
            lat: report.location?.coordinates?.lat || report.lat,
            lng: report.location?.coordinates?.lng || report.lng,
            description: report.description,
            date: new Date(report.createdAt || report.date).toISOString().split('T')[0],
  ...
  useEffect(() => {
    ...
    try {
      reports.forEach((report) => {
        if (map.current && report.lat && report.lng) {
